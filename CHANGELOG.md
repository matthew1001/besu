...
- Add `--p2p-tx-feecap` to cap the maximum fee of transactions received from peers over P2P, mirroring `--rpc-tx-feecap` (which applies to locally-submitted transactions). Defaults to 1 ETH like `--rpc-tx-feecap`. A value of 0 caps fees to 0, rejecting any transaction with a positive gas price.
