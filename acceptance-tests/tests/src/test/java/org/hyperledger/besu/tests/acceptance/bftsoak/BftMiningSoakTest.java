...
public class BftMiningSoakTest extends ParameterizedBftTestBase {
  ...
  private int testRunTimeMins = 30;
  ...
  public void shouldBeStableDuringLongTest(
    ...
    assertThat(System.getProperty("soaktimemins")).isNotNull();
    if (System.getProperty("soaktimemins") != null) {
      testRunTimeMins = Integer.parseInt(System.getProperty("soaktimemins"));
      assertThat(testRunTimeMins).isGreaterThanOrEqualTo(20);
    }
    ...
    assertThat(testRunTimeMins / NUM_STEPS).isGreaterThanOrEqualTo(3);
    ...
    Instant nextStepEndTime = startTime.plus(testRunTimeMins / NUM_STEPS, ChronoUnit.MINUTES);
    ...
    nextStepEndTime = previousStepEndTime.plus(testRunTimeMins / NUM_STEPS, ChronoUnit.MINUTES);
    while (System.currentTimeMillis() < nextStepEndTime.toEpochMilli()) {
      try {
        Thread.sleep(60000);
        chainHeight = minerNode1.execute(ethTransactions.blockNumber());
